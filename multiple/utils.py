...
def read_lookup_table(file_path):
    ...
    lookup_dict = dict(zip(lookup_df['TF'], lookup_df['Normalized_Count']))
    return lookup_dict

def read_lookup_table_target(file_path):
    lookup_df = pd.read_csv(file_path)
    lookup_dict = dict(zip(lookup_df['Gene'], lookup_df['Normalized_Count']))
    return lookup_dict