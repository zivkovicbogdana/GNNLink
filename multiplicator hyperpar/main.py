...
def Main(multiplication_factors, seed=123):
    ...
    for multiplication_factor in multiplication_factors:
        ...
        geneName, feature, logits_train, logits_test, logits_validation, train_mask, test_mask, validation_mask, interaction, neg_logits_train, neg_logits_test, neg_logits_validation, train_data, validation_data, test_data = load_data1(multiplication_factor)
        ...
        with tf.compat.v1.Session() as sess:
            ...
            for epoch in range(epochs):
                ...
                print(f"Epoch {epoch}: avg_auc_val = {avg_auc_val}, multiplication_factor = {multiplication_factor}")
...
multiplication_factors = [0, 1, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000]  # List of factors to test
for t in range(T):
    aupr_vec = []
    auroc_ver = []
    ...
    # Retrain with the best multiplication factor
    print("Retraining with the best multiplication factor")
    tf.compat.v1.reset_default_graph()
    np.random.seed(seed)
    tf.compat.v1.set_random_seed(seed)

    geneName, feature, logits_train, logits_test, logits_validation, train_mask, test_mask, validation_mask, interaction, neg_logits_train, neg_logits_test, neg_logits_validation, train_data, validation_data, test_data = load_data1(best_factor)
    biases = preprocess_graph(interaction)
    model = TransorfomerModel(feature, do_train=False)
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for epoch in range(epochs):
            ...
            print(f"Epoch {epoch}: Training: loss = {train_loss_avg:.5f}, acc = {train_acc_avg:.5f}, time = {time.time() - t:.5f}")

        print("Finish retraining for factor:", best_factor)
        ...
        print("Start to test with the retrained model")
